                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-3 mt-auto">
                      {portal.tags.map((tag, i) => (
                        <span key={`tag-${portal.name}-${tag}-${i}`} className="px-3 py-1 bg-white text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] rounded-lg border border-gray-100 premium-shadow">
                          {tag}
                        </span>
                      ))}
                    </div>
              ...
              <div className="relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {expatGuides.filter(g => g.category === 'Lifestyle').map((guide, i) => (
                    <motion.div 
                      key={`lifestyle-guide-${guide.id}-${i}`} 
                      whileHover={{ x: 10 }}
                      onClick={() => setSelectedGuide(guide)}
